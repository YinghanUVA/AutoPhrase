...
for item in "nus" "inspec" "krapivin" "semeval"
do
    TEXT_TO_SEG=./data/EN/$item.test.100aug.txt
    echo $TEXT_TO_SEG
    ...
    time java $TOKENIZER -m segmentation -i $TEXT_TO_SEG -segmented tmp/tokenized_segmented_sentences.txt -o ${MODEL}/segmentation.$item.100aug.txt -tokenized_raw tmp/raw_tokenized_text_to_seg.txt -tokenized_id tmp/tokenized_text_to_seg.txt -c N
